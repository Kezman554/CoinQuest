...
- **The PIN is verified server-side and rejected server-side.** Never returned to a client, never embedded in the bundle. Hiding a button is presentation, not authorisation. Consecutive failures are counted per source address and cool off on their own — see `app/services/lockout.py` for what per-source costs

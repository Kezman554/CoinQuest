...
- **The PIN is verified server-side and rejected server-side.** Never returned to a client, never embedded in the bundle. Hiding a button is presentation, not authorisation. Consecutive failures are counted per source address and cool off on their own, escalating 30s to 15min and resetting on a correct PIN — see `app/services/lockout.py` for what per-source costs on a screen the whole household shares
- **The PIN guards what gives money, not what proposes losing it.** Confirming, rejecting, settling and clearing a miss carry it. Claiming and marking a chore missed do not: both are proposals that pay nothing, and a wall screen asking for four digits defeats the ten seconds a parent has while noticing a miss. The asymmetry is the point — anything that costs him money is a tap, anything that gives it back is a parent (Session Y; the rule it narrows is quoted in `app/routers/claims.py`)
